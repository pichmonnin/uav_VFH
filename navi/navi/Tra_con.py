...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        self.arm_service = self.create_service(
            Trigger, 'arm_vehicle', self.arm_callback)
        self.disarm_service = self.create_service(
            Trigger, 'disarm_vehicle', self.disarm_callback)
        self.land_service = self.create_service(
            Trigger, 'land_vehicle', self.land_callback)
        self.stop_setpot_service = self.create_service(Trigger , "stop_setpoint_publishing" ,self.step_setpoint_callback)
    def step_setpoint_callback(self, request , response):
        self.is_publishing_setpoints = not self.is_publishing_setpoints
        status = "resumed" if self.is_publishing_setpoints else "stopped"
        self.get_logger().info(f"Setpoints publishing has been {status}")
        response.success = True
        response.message = f"SetPoint publishing {status}"
        return response
        
    def arm_callback(self, request, response):
        """Service callback for arming the vehicle."""
        self.arm()
        response.success = True
    ...
    def disarm_callback(self, request, response):
        """Service callback for disarming the vehicle."""
        self.disarm()
        response.success = True
    ...
    def land_callback(self, request, response):
        """Service callback for landing the vehicle."""
        self.land()
        response.success = True
